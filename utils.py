...
client = OpenAI(api_key="type your api key")
...
Give the exact service name that fits, and a brief justification of how Ergosign can help with that service.
...
async def map_all_opportunities(df):
    ...
    df.to_excel("mapped_services.xlsx", index=False)
...
async def translate_digitalization_findings(rows, source_lang="auto"):
    translated = []
    for row in rows:
        for col in ['evidence', 'area', 'digital_solution', 'reason']:
