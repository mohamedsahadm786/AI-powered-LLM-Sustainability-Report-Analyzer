...
async def run_pipeline(pdf_path, output_queue=None):
    ...
    for chunk_start in range(0, total_pages, chunk_size):
        ...
        if extracted_rows:
            ...
            msg2 = f"🎯 Successfully Processed the Pages {chunk_start+1}-{chunk_end}:"
            print("\n" + msg2)
            if output_queue is not None:
                await output_queue.put(("update", msg1 + "\n" + msg2))
    ...
    if all_findings:
        ...
        merged_df.to_excel("opportunities.xlsx", index=False)
        ...
        empty_df.to_excel("opportunities.xlsx", index=False)
